<?php
	...
	class ListModel
	{
		...
		public static function create_list(string $name, string $userID, ?string $color = NULL) : object|array {
			$newList = Database::execute_query("INSERT INTO lists.list (id, userid, name) VALUES (?,?,?) RETURNING id",
				[ Uuid::uuid4(), $userID, $name ], true);

			if ( !is_null($color) && isset($newList->id) ) {
				Database::execute_query("INSERT INTO lists.list_color (userid, listid, color) VALUES (?,?,?) ON CONFLICT (userid,listid) DO UPDATE SET color = excluded.color",
					[ $userID, $newList->id, $color ]);
			}
		...
		public static function user_in_list(string $listID, string $userID) : object|array {
			return Database::execute_query("SELECT * FROM lists.list_user WHERE listid = ? AND userid = ?",
				[ $listID, $userID ], true);
		}
		...
		public static function remove_user_from_list(string $listID, string $userID) {
			Database::execute_query("DELETE FROM lists.list_user WHERE listid = ? AND userid = ?",
				[ $listID, $userID ]);
		}
		...
		public static function update_list_color(string $listID, ?string $color = NULL) {
			if ( !is_null($color) ) {
				$userID = $_SESSION['userID'];
				Database::execute_query('INSERT INTO lists.list_color (userid, listid, color) VALUES (?,?,?) ON CONFLICT (userid,listid) DO UPDATE SET color = excluded.color',
					[ $userID, $listID, $color ]);
